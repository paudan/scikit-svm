...
public:
	...
	double *C_, double eps, int shrinking, int nr_class, int solver_type);
private:
	int active_size;
	...
	double get_bound(int i, int m)
	{
		if (y[i] == m)
			return upper_bound[m];
		return 0;
	}
	void update_alpha_status(int i, int m)
	{
		if(alpha[i*nr_class+m] >= get_bound(i, m))
			alpha_status[i*nr_class+m] = false;
		else alpha_status[i*nr_class+m] = true;
	}
	void swap_index(int i, int j);
	double select_working_set(int &q);
	void solve_sub_problem(double A, double *B, double C, int yi, double *alpha_new);
...
void Solver_SPOC::reconstruct_gradient()
{
	...
	for (i=active_size;i<l;i++)
		G[i*nr_class+y[i]] = alpha[i*nr_class+y[i]]*diag[y[i]];
	...
	double *C_, double eps, int shrinking, int nr_class,int solver_type)
{
	...
	this->eps = eps;
	unshrinked = false;
	diag = Malloc(double,l);
	upper_bound = Malloc(double,nr_class);
	int i, m, q, old_q = -1;
	if (solver_type == SPOC_L2)
	{
		for (i = 0; i<l; i++)
			diag[i] = 0.5/C[i];
	...
	while (1)
	{
		...
		B[y[q]] -= alpha[q*nr_class+y[q]]*diag[y[q]];

		if (A > 0)
			solve_sub_problem(A, B, C[y[q]], y[q], nu);
		else
		{
			i = 0;
			if (y[q] == 0)
			...
			if (B[y[q]]-B[i] < -A*C[y[q]])
				nu[y[q]] = C[y[q]];
			if (solver_type == SPOC_L2)
			{
				double A_s = A + 0.25 / C[y[q]];
				if (A_s <= 0)
				{
					fprintf(stderr,"Error: sub-problem not solvable when K(i,i) <= -C/4\n");
					exit(1);
				}
				nu[y[q]] = -0.5 * (B[y[q]] - B[i]) / A_s;
			}
			nu[i] = -nu[y[q]];
		}
...
void Solver_SPOC::solve_sub_problem(double A, double *B, double C, int yi, double *alpha_new)
{
	int r;
	double *D;
	double A_yi = A+this->diag[yi];
	double A_over_Ayi=A/A_yi;
	int shift_index=0;
	clone(D, B, nr_class+1);
	D[yi]+=A*C;
	if (this->upper_bound[yi]==INF)
	{
		shift_index=1;
		swap(D[0],D[yi]);
	}
	qsort(&D[shift_index], nr_class-shift_index, sizeof(double), compar);
	D[nr_class] = -INF;
	double beta = (D[0] - A*C)*A_over_Ayi;
	A_over_Ayi--;
	for (r=1;beta<(A_over_Ayi+r)*D[r];r++)
		beta += D[r];
	delete[] D;

	beta /= (A_over_Ayi+r);
	for (r=0;r<nr_class;r++)
		alpha_new[r] = min((double) 0, (beta - B[r])/A);
	alpha_new[yi] = min(this->upper_bound[yi],(beta-B[yi])/A_yi);
}
...
decision_function solve_spoc(const svm_problem *prob, 
	const svm_parameter *param, int nr_class, double *weighted_C, int solver_type)
{
	int i, m, l = prob->l;
	...
	param->eps, param->shrinking, nr_class, solver_type);
...
svm_model *svm_train(const svm_problem *prob, const svm_parameter *param)
{
	...
	if (param->svm_type == SPOC || param->svm_type == SPOC_L2)
	{
		
		f = solve_spoc(&sub_prob,param,nr_class,weighted_C,param->svm_type);
		delete[] sub_prob.y;
...
double svm_predict(const svm_model *model, const svm_node *x)
{
	...
	if (model->param.svm_type == SPOC || model->param.svm_type == SPOC_L2)
	{
...
{
	"c_svc","kbb","spoc","epsilon_svr","spoc_l2",NULL
};
...
out:
	...
	int m = (param.svm_type == SPOC || param.svm_type == SPOC_L2) ? model->nr_class : model->nr_class - 1;
	int l = model->l;
